...
<head>
	...
	<?php endif; ?>		
	#wrapper{
		max-width:<?php if( $site_width_percent ) { echo $smof_data['site_width']; } else { echo ( $site_width + 100 ) .  'px'; } ?>;
		margin:0 auto;
	}
	...
	@media (min-width: 1014px) {
		body #header-sticky.sticky-header {
			width:<?php if( $site_width_percent ) { echo $smof_data['site_width']; } else { echo ( $site_width + 100 ) .  'px'; } ?>;
			left: 0;
			right: 0;
			margin:0 auto;
		}	
	...
	@media only screen and (min-device-width: 768px) and (max-device-width: 1366px) and (orientation: portrait){
		#nav ul li, #sticky-nav ul li { padding-right: <?php echo $smof_data['mobile_nav_padding']; ?>px !important; }
	}
	...
	<?php if( ( ($smof_data['header_transparent'] && get_post_meta($object_id, 'pyre_transparent_header', true) != 'no' ) || ( ! $smof_data['header_transparent'] && get_post_meta($object_id, 'pyre_transparent_header', true) == 'yes' ) ) 
			  ...
			  && ! is_author()
			  && ! is_archive()
			  && get_post_meta($object_id, 'pyre_slider_type', true) != 'no'
	): ?>
	...
	foreach( $gfont as $g_font ) {
		echo "<link href='http://fonts.googleapis.com/css?family={$g_font}:" . $smof_data['gfont_settings'] . "' rel='stylesheet' type='text/css' />";
	}
	?>
	<?php endif; ?>
